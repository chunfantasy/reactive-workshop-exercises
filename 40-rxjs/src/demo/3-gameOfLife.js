...
const gameOfLife = Observable.merge(
  Observable.fromEvent(document.getElementById('grid'), 'click')
    .map(({target}) => ({
      type: 'toggle',
      row: parseInt(target.getAttribute('data-row'), 10),
      column: parseInt(target.getAttribute('data-column'), 10)
    })),
  Observable.fromEvent(document.getElementById('tick'), 'click')
    .map(e => ({type: 'tick'}))
).scan(
  (currentState, {type, row, column}) => {
    if (type === 'toggle') {
...
gameOfLife.subscribe(isAlive => Array.prototype.forEach.call(cells, cell => cell.classList.toggle('alive', !!isAlive[cellKey(cell.getAttribute('data-row'), cell.getAttribute('data-column'))])));
